...
func initSubErrors() {
	SubErrors = [_SubModuleErrMax][]string{
		...
		MTokenErr: {
			...
			TokenDecryptErr:    "Token decoding failure.",
		...
		MCaptchaErr: {
			...
			CaptchaWriteImageErr: "Generate graphics validation code audio failure.",
			CaptchaWriteAudioErr: "Failed to obtain graphical verification code.",
...
func initSubModuleErrs() {
	SubModuleErrs = [_SubModuleErrMax]string{
		_SubModuleErrMin: "_SubModuleErrMin", // 占位
